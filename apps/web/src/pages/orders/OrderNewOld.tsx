...
// ===============================
// Map component with clearMarkers
// ===============================
type OrderMapHandle = {
...
// replace your existing OrderMap with this
...
const OrderMap = forwardRef<OrderMapHandle, OrderMapProps>(function OrderMap(
  ...
  // NEW: keep latest props in refs so init effect can stay []
  ...
  // ✅ init ONCE; no deps on active/onSet so the map doesn't reset
  useEffect(() => {
    ...
    const clickHandler = async (e: any) => {
      ...
      const which = activeRef.current;          // read latest active
      const setFn = onSetRef.current;           // read latest onSet
  ...
  // reflect incoming coords on existing markers and line (safe to depend on lat/lng only)
  useEffect(() => {
    const map = mapRef.current;
    if (!map) return;
...
export default function OrderNew() {
  ...
  const [mode, setMode] = useState<"LEAVE_NOW" | "SCHEDULED">("LEAVE_NOW");
  ...
  // layout/map state
  const [showMap, setShowMap] = useState<boolean>(false); // right pane collapsed by default
  ...
  return (
    <div className="two-col edge-to-edge" style={{ minHeight: "100vh" }}>
      ...
      <style>{`
        /* Make the two-col container span full viewport width even if parent has padding */
        .edge-to-edge {
  width: 100vw;
  margin-left: calc(50% - 50vw);
  margin-right: calc(50% - 50vw);
  padding-inline: 12px;      /* add a tiny left/right gutter */
  box-sizing: border-box;     /* prevents horizontal scroll with 100vw */
}
        .left-pane, .right-pane { padding: 0; }
        .left-pane { flex: 1 1 50%; }
        .right-pane { flex: 1 1 50%; background: #f7f7fb; }

        /* Horizontal label + field */
        .field-row {
          display: grid;
        ...
        /* Keep your existing grid helpers working */
        .grid2 { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
        .grid3 { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; }
        .grid4 { display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 12px; }

        /* Remove default card rounding/margins so it hits the left edge */
